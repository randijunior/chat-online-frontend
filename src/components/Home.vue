...
export default {
  ...
  sockets: {
    ...
    messageReceived: function (data) {
            var chatListM = this.chatList.find((e) => e.id == data[0].user_id);
            
      if(typeof chatListM == 'undefined') {
              chatListM = {
            id: data[0].user_id,
            friend: {
              ...
              _id: data[1]._id,
              friend_image: data[1].user_image
            },
            messages: [data[0]],
            unread_messages: 0,
            new_chat: true
      ...
      let friend = this.friends.find(
        (element) => element.friend_id == data[1]._id
      );
      if (!friend) {
      ...
      if(chatListM.new_chat)this.chatList.push(chatListM)
    },
  },
  methods: {
    onClickFriendChatListener(id, isNewChat) {
      if (isNewChat) {
        ...
        if (this.user.id == id.friend_id)
          this.currentChat.messages = this.messages.filter(
            (m) => m.friend_id == this.user.id
          );
        ...
        if (id.id == this.user.id) {
          this.currentChat.messages = id.messages.filter(
            (m) =>
              Object.prototype.hasOwnProperty.call(m, "__v") &&
              m.friend_id == this.user.id &&
    ...
    getUserDetails() {
      ...
      try {
        ...
        console.error(error, "erro ao decodificar token");
      }
    },
    getFriendMessages(friendID) {
      return this.messages.filter(
        (message) =>
          message.friend_id == friendID || message.user_id == friendID
      );
    },
    ...
    changeFriendStatus(status, id) {
      ...
      let indexFriendList = this.friends.findIndex(
        (element) => element.friend_id === id
      );

      //adiciona propriedade online ao amigo
      if (indexFriend !== -1)
        this.chatList[indexFriend].friend["online"] = status;
      if (indexFriendList !== -1) this.friends[indexFriendList].online = status;
    },
    getLastMessage(id) {
    ...
    HTTP.get("/messages")
      .then(async (response) => {
        ...
        for (let x = 0; x < this.messages.length; x++)
          ids.push(this.messages[x].user_id);
          this.messages.filter(m => m.user_id != m.friend_id && m.date)
          .forEach(v => {
            idsUnread.push(v.friend_id)
          })
          let uniquesUnread = [...new Set(idsUnread)]
        let uniques = [...new Set(ids)];
        uniquesUnread.forEach(value => {
          HTTP.get(`users/${value}`).then(r => {
            let obj = {
                id: value,
                friend: {
                  ...
                  _id: r.data._id,
                  friend_image: r.data.user_image,
                },
                messages: this.getFriendMessages(value)
              }
        ...
        for (let x = 0; x < uniques.length; x++) {
          ...
          if (uniques[x] == this.user.id) {
            ...
            if (
              this.messages.findIndex((m) => m.user_id == msg.friend_id) == -1
            ) {
              let mObj = this.messages.find(
                (message) => message.user_id == uniques[x]
              );
              ...
              if(!this.chatList.find(c => c.id == obj.id))this.chatList.push(obj);
            } else if (this.messages.find((m) => m.user_id === m.friend_id)){
            ...
            let unr = f_m.filter(
              (el) => el.readed == false && el.user_id != this.user.id
            ).length;
            let obj = {
              id: uniques[x],
              friend: {
